      ...
      <div ref={ref} className={`max-w-7xl mx-auto px-4 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-green-400/20 bg-green-500/10 backdrop-blur-sm mb-6">
            <span className="text-sm font-medium text-green-300">Tarifs transparents</span>
          </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative rounded-3xl p-8 border transition-all duration-300 ${
                plan.featured
                  ? 'bg-gradient-to-b from-blue-600/20 to-indigo-600/20 border-blue-500/40 shadow-xl shadow-blue-600/10 scale-105 md:scale-105'
            ...
            >
              ...
              <Link
                ...
                className={`block text-center py-3.5 px-6 rounded-xl font-bold text-sm transition-all ${
                  plan.featured
                    ? 'bg-white text-[#0a0e27] hover:bg-gray-100 shadow-lg'
                    : 'border border-gray-700 text-gray-300 hover:bg-white/5 hover:text-white'
                }`}
              >
